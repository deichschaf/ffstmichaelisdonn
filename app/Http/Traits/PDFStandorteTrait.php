...
trait PDFStandorteTrait
{
    ...
    {
        $config = PDFStandorteTrait::getConfig();
        $pdf->AddPage();
        ...
        $pdf->Cell($config ['pdf_zeile'], $config ['pdf_hoehe'], $pdf->UTF8Text('Standorte nach Funkrufnamen'), 0, 1, 'C');
    ...
    private static function getStandorte($pdf)
    {
        $config = PDFStandorteTrait::getConfig();
        $pdf = PDFStandorteTrait::makeAddHead($pdf, $config);
        $pdf = PDFStandorteTrait::getRufnamen($pdf);
        $pdf = PDFStandorteTrait::getHiOrgs($pdf);
        return $pdf;
    }
    ...
    public static function getStaticPdf($file)
    {
        ...
        $pdf = PDFStandorteTrait::getStandorte($pdf);

        // PDF Output
        if ($file === "pdf") {
            $pdf->Output('Standorte.pdf', 'D'); // Dokument wird an den Browser geschickt
        } else {
            $pdf->Output('Standorte.pdf', 'I'); // Dokument wird an den Browser geschickt
        }
    }
}
